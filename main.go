...
func main() {
	...
	// 读取文本文件中的ID列表
	hfpiaoFile, err := os.Open("hfpiao_channels.log")
	if err != nil {
		fmt.Println("无法打开文件:", err)
		return
	}
	...
	hfpiaoScanner := bufio.NewScanner(hfpiaoFile)
	for hfpiaoScanner.Scan() {
		id := hfpiaoScanner.Text()
		ids = append(ids, id)
	}
	if err := scanner.Err(); err != nil {
		fmt.Println("读取文件时出错:", err)
	...
	tvXml, err := ioutil.ReadFile("meroser.xml")
	if err != nil {
		fmt.Println("无法打开文件:", err)
		return
	}
	
	var root,tvxmlroot, tempRoot Tv
	xml.Unmarshal(epgData, &root)
	xml.Unmarshal(tvXml, &tvxmlroot)
	...
	for _, channel := range tvxmlroot.Channel {
		channelMap[channel.ID] = channel
	}
	...
	for _, v := range ids {
		...
		for _, programme := range tvxmlroot.Programme {
			if programme.ChannelId == v {
				tempRoot.Programme = append(tempRoot.Programme, programme)
			}
		}
